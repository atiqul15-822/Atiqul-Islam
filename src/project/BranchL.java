package project;
import java.sql.*;
public class BranchL extends javax.swing.JFrame {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try{
            ...
            String query = "select Branch_ID,Password from Branch where Branch_ID = "+jTextField1.getText();
            Statement stmnt = con.createStatement();
            ResultSet rs=stmnt.executeQuery(query);
            System.out.println(query);
            rs.next();
            String pass = rs.getString(2);
            if(pass.compareTo(jTextField2.getText())==0) {
                this.setVisible(false);
                new BranchLogin(rs.getInt(1)).setVisible(true);
            } else {
                this.setVisible(false);
                new WrongIdOrPassword(1).setVisible(true);
            }
        }
        catch(Exception e) {
            this.setVisible(false);
